...
use App\Models\Toko;   // Pastikan Model ini ada (sesuai langkah sebelumnya)
...
class LandingController extends Controller
{
    ...
    {
        $user = Auth::user(); // Ganti $_SESSION['user']
        $cityId = 0;
        $districtId = 0;
        $tokoSectionTitle = "Toko Populer Nasional";

        // 1. Cek Lokasi User (Jika Login)
        if ($user) {
            // Asumsi tabel alamat user: tb_user_alamat
        ...
        // Filter Toko Berdasarkan Lokasi (Jika ada)
        if ($cityId > 0) {
            ...
            // Urutkan berdasarkan produk terbanyak & nama
    ...
    private function getBestSellingProducts($cityId = null, $districtId = null)
    {
        ...
        // Subquery untuk hitung total terjual
        $query->orderByDesc(function ($query) {
            $query->selectRaw('sum(jumlah)')
